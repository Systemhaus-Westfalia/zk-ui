...
import org.adempiere.apps.form.AllocationSHW;
 ...
 */
public class WAllocationSHW extends AllocationSHW
	implements IFormController, EventListener, WTableModelListener, ValueChangeListener
{
	...
	private Label bpartnerLabelpayment = new Label();
	private WSearchEditor bpartnerSearch = null;
	private WSearchEditor bpartnerPaymentSearch = null;
	...
	private void zkInit() throws Exception
	{
		...
		bpartnerLabelpayment.setText(" " + Msg.translate(Env.getCtx(), "SdN Pago"));
		...
		row = rows.newRow();
		row.appendChild(bpartnerLabelpayment.rightAlign());
		row.appendChild(bpartnerPaymentSearch.getComponent());
	...
	public void dynInit() throws Exception
	{
		...
		AD_Column_ID = 3499;        //  C_Payment.C_BPartner_ID

		MLookup lookupBPPayment =MLookupFactory.get (Env.getCtx(), getWindowNo(), 0, AD_Column_ID, DisplayType.Search);
				
		//MLookup lookupBPInvoice = MLookupFactory.get (Env.getCtx(), m_WindowNo, 0, AD_Column_ID, DisplayType.Search);
		
		bpartnerPaymentSearch = new WSearchEditor("C_BPartnerPayment_ID", true, false, true, lookupBPPayment);
		bpartnerPaymentSearch.addValueChangeListener(this);
	...
	public void valueChange (ValueChangeEvent e)
	{
		...
		if (name.equals("C_BPartner_ID"))
		{
			...
			else
			{
				...
				//SHWbPartnerId
				bpartnerPaymentSearch.setValue(value);
				bPartnerPaymentId = ((Integer)value).intValue();
				loadBPartner();
			}
		...
		if (name.equals("C_BPartnerPayment_ID"))
		{
			bpartnerPaymentSearch.setValue(value);
			bPartnerPaymentId = ((Integer)value).intValue();
			loadBPartner();
