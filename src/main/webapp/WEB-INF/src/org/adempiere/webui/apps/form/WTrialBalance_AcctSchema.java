...
import org.compiere.apps.form.TrialBalanceDrill_AcctSchema;
 ...
 */
public class WTrialBalance_AcctSchema extends TrialBalanceDrill_AcctSchema implements IFormController, EventListener, ValueChangeListener
{
	private CustomForm		form			= new CustomForm();
	/** Logger */
	private static CLogger	log				= CLogger.getCLogger(WTrialBalance_AcctSchema.class);
	...
	private Label			lAcctSchema			= new Label(Msg.translate(Env.getCtx(), "C_AcctSchema_ID"));
	private WTableDirEditor	fAcctSchema			= null;
	...
	public WTrialBalance_AcctSchema()
	{
		try
		{
			super.dynInit();
	...
	private void init()
	{
		...
		row.appendChild(lAcctSchema.rightAlign());
	...
	protected void dynInit() throws Exception
	{
		...
		MLookup lookupAcctSchema = MLookupFactory.get(Env.getCtx(),
				form.getWindowNo(),
				2513,
				DisplayType.Table,
				Env.getLanguage(Env.getCtx()),
				...
				136,
				false,
				"C_AcctSchema.IsActive='Y' AND C_AcctSchema.AD_Client_ID="+m_AD_Client_ID );
		fAcctSchema = new WTableDirEditor("C_AcctSchema_ID", false, false, true, lookupAcctSchema);
	...
	private void loadTable()
	{
		...
		if (c_AcctSchema_ID != 0) {
			acctSchema = MAcctSchema.get(Env.getCtx(), (Integer)fAcctSchema.getValue());
		}
		else {
			acctSchema = MAcctSchema.getClientAcctSchema(Env.getCtx(), Env.getAD_Client_ID(Env.getCtx()))[0];
		}
		if (acctSchema == null)
...
//			FDialog.info(0, null, Msg.getMsg(Env.getCtx(), "SelectExisting"));
	...
	public void valueChange(ValueChangeEvent evt)
	{
		...
		else if(evt.getSource() == fAcctSchema) {
			c_AcctSchema_ID = (Integer)fAcctSchema.getValue();
			fieldAcctFrom.actionRefresh();
			fieldAcctTo.actionRefresh();
		}
	}
