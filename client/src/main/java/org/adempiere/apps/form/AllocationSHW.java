...

{
	...
	{		
		...
		try
		{
			...
			pstmt.setInt(5, bPartnerPaymentId);
			if (!isMultiCurrency)
	...
	public String saveData(int m_WindowNo, Object date, IMiniTable payment, IMiniTable invoice, String trxName) {
		...
		for (MAllocationLine alo:alloc.getLines(true)){
			if (alo.getC_Payment_ID() != 0 && alo.getC_Invoice_ID()!= 0)
			{
				if (alo.getC_Payment().isPrepayment() && alo.getC_Payment().getC_BPartner_ID() != alo.getC_Invoice().getC_BPartner_ID()
						&& alo.getC_Payment().getC_Order_ID() == 0)
				{
					MBPartner bp = (MBPartner)alo.getC_Payment().getC_BPartner();
					BigDecimal openbalance = bp.getTotalOpenBalance();
					bp.setTotalOpenBalance(openbalance.add(alo.getAmount()));
					bp.saveEx();
				}
			}
		}
		String description = alloc.getDocumentNo();
		String newpay = Env.getContext(Env.getCtx(), alloc.getDocumentNo());
		if (newpay.length()!=0)
			description = description + " Reembolso:"+newpay;
		return description;
	}   //  saveData
}
